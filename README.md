...
### 自動コミット（GitHub Actions）
GitHub Actions により、10分ごとに自動コミットが実行されます。
...
**重要:**
- すべてのチームメンバーに影響します（リポジトリの全員に適用）
- 手動コミットと自動コミットが混在します
- コミット履歴が多くなりますが、`git log` で履歴確認可能
- 自動コミット中に `git push` すると競合する可能性があるので注意
